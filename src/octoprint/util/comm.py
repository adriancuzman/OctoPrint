...
class MachineCom(object):
	...
	def _openSerial(self):
		def default(_, port, baudrate, read_timeout):
			...
			self._log("setting pyserial settings")
			serial_obj.port = port
			serial_obj.baudrate = 500000
			serial_obj.writeTimeout = 5
			...
			self._log("setting pyserial settings done")
			self._log(serial.VERSION)
		...
		while self._send_queue_active:
			try:
				...
				try:
					...
					if self.isSendingFileToSDWithSoftwareFlow():
